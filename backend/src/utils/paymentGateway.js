...
export const simulatePayoutGateway = async ({ withdrawalId, amountPaise, userId, forceStatus }) => {
  const delay = Math.floor(Math.random() * 250) + 50;
  await new Promise((resolve) => setTimeout(resolve, delay));

  const isSuccess = forceStatus === 'SUCCESS' ? true : forceStatus === 'FAILED' ? false : Math.random() < 0.7; 
